...
@section('content')
    <div class="min-h-screen flex flex-col items-center justify-center bg-white text-gray-900">
        <header class="w-full py-6 px-8 bg-white shadow-md fixed top-0 z-50 flex justify-between items-center">
            ...
            <nav>
                <ul class="flex space-x-9">
                    ...
                    <li>
                        <a href="{{ asset('assets/cv/cv_nabil.pdf') }}"
        ...
        {{-- Portofolio --}}
        <section id="portofolio" class="w-full flex flex-col items-center justify-center text-center py-16 px-4">
            <h2 class="text-3xl font-bold text-green-600">Portofolio</h2>
            <h4 class="text-lg text-black-600 mt-4 max-w-3xl">Work-Create-Repeat. Here’s a snapshot of my finest digital
                projects</h4>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-8 max-w-4xl w-full">
                @forelse($portfolios as $portfolio)
                    <div class="bg-green shadow-lg rounded-md overflow-hidden transition transform hover:scale-105">
                        {{-- Thumbnail --}}
                        @if ($portfolio->image)
                            <img src="{{ asset('storage/' . $portfolio->image) }}" alt="{{ $portfolio->title }}"
                                class="w-full h-36 object-cover rounded m-2">
                        @endif
                        ...
                        <div class="p-4">
                            ...
                            @if ($portfolio->tech_stack)
                                <p class="text-xs text-green-600 mt-1">Tech: {{ $portfolio->tech_stack }}</p>
                            @endif

                            {{-- Tombol detail --}}
                            <a href="{{ route('portfolio.show', $portfolio->id) }}"
                                class="mt-3 inline-block px-3 py-1.5 bg-green-500 text-white text-sm rounded-lg shadow-md hover:bg-green-600">
                                View Details
                            </a>
                        </div>
                    </div>
